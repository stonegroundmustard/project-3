      ...
      <Container>
        <h2>
          {userData.savedBooks.length
            ? `Viewing ${userData.savedMovies.length} saved ${userData.savedBooks.length === 1 ? 'movie' : 'movies'}:`
          ...
          {userData.savedMovies.map((movie) => {
            return (
              <Card key={movie.movieId} border='dark'>
                {book.image ? <Card.Img src={movie.image} alt={`The cover for ${movie.title}`} variant='top' /> : null}
                <Card.Body>
